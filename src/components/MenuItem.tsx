...
function MenuItem({ id, image, itemName, description, price }: MenuItemProps) {
    ...
    >
      ...
      <div className={cn('flex w-full flex-col', { 'p-4': enlarged })}>
        <div className="flex items-baseline gap-2">
          <h2 className={cn('text-lg font-bold text-cyan-800', { 'text-white': enlarged })}>{itemName}</h2>
          <div className={cn('flex-grow border-b-2 border-dotted', { invisible: enlarged || !price })}></div>
          {price !== 0 && (
            <p className={cn('text-lg font-bold text-cyan-800', { 'text-white': enlarged })}>
        ...
        {description && (
          <div>
            <p className={cn('text-sm text-neutral-700', { 'text-cyan-50': enlarged })}>{description}</p>
          </div>
        )}
      </div>
    </div>
